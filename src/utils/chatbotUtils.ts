const HUGGING_FACE_API_URL = 'https://api-inference.huggingface.co/models/NoePlantier/CommunityApp/facebook/blenderbot-400M-distill';
const API_KEY = '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'; 


interface CommunityAppResponse {
  response: string;
  suggestions?: string[];
}

const CommunityAppResponses: Record<string, CommunityAppResponse> = {
  // Questions sur le concept
  'comment': {
    response: "CommunityApp est un réseau social unique qui favorise les rencontres dans la vie réelle ! Contrairement aux apps classiques, nous organisons des événements et activités pour que vous puissiez rencontrer naturellement des personnes qui partagent vos centres d'intérêt.",
  ...
  // Questions pratiques
  'inscription': {
    response: "Pour rejoindre CommunityApp, c'est simple : téléchargez l'app, créez votre profil en indiquant vos centres d'intérêt, et commencez à parcourir les événements près de chez vous. La vérification d'identité est requise pour garantir la sécurité de notre communauté.",
    suggestions: ["Vérification d'identité", "Centres d'intérêt", "Trouver des événements"]
  },
  'prix': {
    response: "L'inscription sur CommunityApp est gratuite ! Certains événements sont gratuits, d'autres peuvent être payants selon l'activité (entrée en boîte, resto, etc.). Les prix sont toujours clairement indiqués sur chaque événement.",
  ...
  // Support et aide
  'problème': {
    response: "Je suis désolé que vous rencontriez un problème. Notre support est disponible 24/7 à support@CommunityApp.com ou via le bouton 'Signaler' sur l'événement/profil concerné. Nous interviendrons rapidement !",
...
export async function getBotResponse(input: string): Promise<string> {
  try {
    // Vérifier d'abord les réponses spécifiques à CommunityApp
    const response = processCommunityAppResponse(input);
    ...
    return "Désolé, je rencontre des difficultés techniques. N'hésitez pas à contacter notre support à support@CommunityApp.com";
  }
}

function processCommunityAppResponse(input: string): string | null {
  const normalizedInput = input.toLowerCase();
  
  for (const [keyword, responseObj] of Object.entries(CommunityAppResponses)) {
